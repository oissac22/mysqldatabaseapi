...
api.get('/backup', async (req,res) => {
    ...
    exec(`mysqldump -P ${process.env.MYSQL_PORT || 10073} -u root -f -r "${sqlName}" -p${process.env.MYSQL_ROOT_PASSWORD || 'yahewaai'} --all-databases`, (error, stdout) => {
...
api.get('/backup/:database', async (req,res) => {
    ...
    const sqlName = randName + '.sql'
    const zipName = randName + '.zip'
    exec(`mysqldump -P ${process.env.MYSQL_PORT || 10073} -u root -f -r "${sqlName}" -p${process.env.MYSQL_ROOT_PASSWORD || 'yahewaai'} ${database}`, (error, stdout) => {
        exec(`zip ${zipName} ${sqlName}`, (err, stdout) => {
            fs.unlink(sqlName, (err) => console.error(err) )
            res.sendFile(path.join(pathTmpFiles, zipName), (err) => {
                fs.unlink( zipName , (err) => console.error(err) )
...
api.get('/backup/:database/:table', async (req,res) => {
    ...
    const sqlName = randName + '.sql'
    const zipName = randName + '.zip'
    exec(`mysqldump -P ${process.env.MYSQL_PORT || 10073} -u root -f -r "${sqlName}" -p${process.env.MYSQL_ROOT_PASSWORD || 'yahewaai'} ${database} ${table}`, (error, stdout) => {
        exec(`zip ${zipName} ${sqlName}`, (err, stdout) => {
            fs.unlink(sqlName, (err) => console.error(err) )
            res.sendFile(path.join(pathTmpFiles, zipName), (err) => {
                fs.unlink( zipName , (err) => console.error(err) )
